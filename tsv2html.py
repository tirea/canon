        ...
        with open(infile) as tsv:
            ...
            for line in tsv:
                ...
                if thread.startswith('"') and thread.endswith('"'):
                    thread = thread[1:-1]
                if not thread.startswith("Re:") and not thread.startswith("MOVED:"):
                    html.write(
                        '<li><a href="#{}">{}</a></li>\n'.format(index, thread))
                    index += 1
        html.write('</ul>\n')
...
def bbc2html(text):
    ...
    # fix table errors
    out = out.replace(
        '[table]<br />', '[table]').replace('<br />[/table]', '[/table]')
    ...
    for color in colors:
        out = out.replace('[color={}]'.format(color),
    ...
    for size in range(72):
        out = out.replace('[size={}pt]'.format(
            size), '<span style="font-size:{}pt;">'.format(size))
    ...
    for font in fonts:
        out = out.replace('[font={}]'.format(
            font), '<span style="font-family: {};">'.format(font))
    out = out.replace('[/font]', '</span>')

    # pre left center right
    out = out.replace('[pre]', '<pre>').replace('[/pre]', '</pre>')
    out = out.replace(
        '[left]', '<div style="text-align:left;">').replace('[/left]', '</div>')
    out = out.replace(
        '[center]', '<div style="text-align:center;">').replace('[/center]', '</div>')
    out = out.replace(
        '[right]', '<div style="text-align:right;">').replace('[/right]', '</div>')
    ...
    # code tt
    out = out.replace(
        '[code]', '<pre style="font-family: monospace;">').replace('[/code]', '</pre>')
    out = out.replace(
        '[tt]', '<pre style="font-family: monospace;">').replace('[/tt]', '</pre>')
    ...
    with open(outfile, "a") as html:
        with open(infile) as tsv:
            ...
            for line in tsv:
                ...
                html.write('<br><hr><br>\n')
                html.write(
                    '<a href="https://forum.learnnavi.org/?msg={}">msg={}</a>'.format(msg, msg))
                html.write(' | ')
                html.write(
                    '<a href="https://forum.learnnavi.org/?topic={}">topic={}</a>\t'.format(topic, topic))
                html.write(' | ')
                html.write(
                    '<a href="https://forum.learnnavi.org/?board={}">board={}</a>\t'.format(board, board))
                html.write(' | ')
                html.write('time={}\t'.format(timestamp))
                html.write(' | ')
                html.write(
                    '<a href="https://forum.learnnavi.org/profile/?u={}">u={}</a>\n'.format(u, u))
